...
`ping` `getLicense` `getMusicFolders` `getIndexes` `getArtists` `getArtist` `getAlbum` `getSong` `getAlbumList2` `getGenres` `search3` `stream` `download` `getCoverArt` `getPlaylists` `getPlaylist` `createPlaylist` `updatePlaylist` `deletePlaylist` `getRandomSongs` `scrobble` `star` `unstar` `getStarred` `getStarred2` `setRating` `getLyrics`
...
`getLyrics` looks up plain-text lyrics from [LRCLIB](https://lrclib.net) (a free, keyless lyrics database) by exact `artist`+`title` match, using whatever the client sends for those two params. There's no fuzzy/title-only fallback: with messy tags (e.g. a compilation folder where the artist field is wrong or "Various Artists"), a title-only search tends to return confident-looking wrong matches (covers, live versions, same-titled songs) more often than it helps, so a non-exact match just returns empty lyrics rather than guessing. LRCLIB's own synced (LRC) lyrics are flattened to plain text, since the classic `getLyrics` response has no field for timing data. The bundled Web UI's player bar has a 🎤 button that fetches and shows these lyrics in a modal, lazily (only on click, not on every track change) — see [Web UI](#web-ui) below.
...
The Upload page (nav bar → Upload) and "⬆ Upload here" from any Folders page (opens the same upload form in a modal, pre-filled with the current folder, so uploading doesn't navigate away — the folder view refreshes in place once it's done) both add one track at a time — a dropzone accepts drag-and-drop or a regular file picker. Tags (artist/album/title/year/genre/track/disc number) are read client-side in the browser, via `music-metadata`'s `parseBlob`, and pre-fill an editable form; parsing failures just leave the fields blank (title falls back to the filename) rather than blocking the upload. The Worker itself does no tag parsing — this keeps it dependency-free and running in the plain Workers runtime, same as everywhere else in `src/`.
