...
`getLyrics` 按客户端传来的 `artist`+`title` 去 [LRCLIB](https://lrclib.net)（免费、不用注册的歌词库）做精确匹配，取纯文本歌词。没有模糊/仅按标题搜索的兜底——标签混乱的情况下（比如合辑文件夹里 artist 字段错了，或者干脆是 "Various Artists"），仅按标题搜往往会匹配到"看起来很对但其实是错的"结果（翻唱、live 版、同名歌），所以没精确匹配上就直接返回空歌词，不做猜测。LRCLIB 自带的同步歌词（LRC 格式）会被拍平成纯文本，因为经典版 `getLyrics` 响应里没有时间轴字段。自带 Web UI 的播放条上有个 🎤 按钮，点了才会去拉歌词并弹窗显示（不是每切一首歌就自动拉一次），见下面 [Web UI](#web-ui)。
