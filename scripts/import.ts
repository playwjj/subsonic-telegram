...
// Marker some local library-maintenance tools (e.g. fix_mp3_tags.py's
// `--cover` flag) write into the APIC frame's description field when they
// generate a placeholder cover themselves (a geometric color-block image)
// because no real art was found. Authoritative when present.
const AUTOGENERATED_COVER_DESC = "AutoGenerated";

async function ensureCoverArt(
  albumId: string,
  picture: { data: Uint8Array; format: string; description?: string },
): Promise<void> {
  ...
  if (picture.description === AUTOGENERATED_COVER_DESC) {
    console.log(`  cover is tagged "${AUTOGENERATED_COVER_DESC}" (a generated placeholder), skipping`);
    return;
  }
...
async function main() {
  ...
  for await (const file of walk(MUSIC_DIR)) {
    ...
    try {
      ...
      if (picture) {
        await ensureCoverArt(albumId, { data: picture.data, format: picture.format, description: picture.description });
      }
